...
async def gen_link_s(bot, message):
    ...
    await message.reply(f"<b>⪼ HERE'S YOUR LINK:- </b>\nhttps://telegram.dog/{temp.U_NAME}?start={outstr}")
    
    
@Client.on_message(filters.command(['batch', 'pbatch']) & filters.create(allowed))
async def gen_link_batch(bot, message):
    if " " not in message.text:
        return await message.reply("𝚄𝚂𝙴 𝙲𝙾𝚁𝚁𝙴𝙲𝚃 𝙵𝙾𝚁𝙼𝙰𝚃.\n𝙴𝚇𝙰𝙼𝙿𝙻𝙴 ›› <code>/batch https://telegram.dog/R_MvzZ/10 https://telegram.dog/R_MvzZ/20</code>.")
    links = message.text.strip().split(" ")
    if len(links) != 3:
        return await message.reply("Use correct format.\nExample <code>/batch https://telegram.dog/R_MvzZ/10 https://telegram.dog/R_MvzZ/20</code>.")
    ...
    if chat_id in FILE_STORE_CHANNEL:
        ...
        return await sts.edit(f"<b>⪼ HERE IS YOUR LINK:›› https://telegram.dog/{temp.U_NAME}?start=DSTORE-{b_64}</b>")
